...
public class webSocketServer extends WebSocketServer {
    ...
    // 客户端信息
    private clientInfo clientInfo = new clientInfo("bind", UUID.randomUUID().toString(), "", "targetId");
    ...
    public void onOpen(WebSocket conn, ClientHandshake handshake) {
        if (!isConnected) {
            ...
            client = conn;
            // 生成新的随机客户端ID用于配对
            expectedClientId = UUID.randomUUID().toString();
            clientInfo.setClientId(expectedClientId);
            clientInfo.setTargetId("");
            clientInfo.setMessage("pending");
            client.send(new Gson().toJson(clientInfo, clientInfo.class));
            Minecraft mc = Minecraft.getInstance();
            if (mc.player != null) {
                mc.player.sendOverlayMessage(Component.literal("==="));
                mc.player.sendOverlayMessage(Component.literal("作者不对使用此模组造成的人身伤害和精神伤害负责"));
                mc.player.sendOverlayMessage(Component.literal("使用此模组请自行注意人身安全"));
                mc.player.sendOverlayMessage(Component.literal("==="));
            }
    ...
    public void onClose(WebSocket conn, int code, String reason, boolean remote) {
        if (conn.equals(client)) {
            isConnected = false;
            client = null;
            clientInfo = new clientInfo("bind", UUID.randomUUID().toString(), "", "targetId");
    ...
    public void onMessage(WebSocket conn, String message) {
        ...
        clientInfo tmp;
        try {
            tmp = new Gson().fromJson(message, clientInfo.class);
        ...
        // 验证反序列化结果
        if (tmp == null || tmp.getType() == null || tmp.getMessage() == null) {
            conn.send("{\"type\":\"error\",\"message\":\"invalid_message\"}");
            return;
        }

        // 动态配对逻辑 - 客户端发送的targetId与服务器的clientId匹配时建立连接
        if (tmp.getMessage().equals("DGLAB") && tmp.getType().equals("bind") 
            && tmp.getTargetId() != null && tmp.getTargetId().equals(expectedClientId)) {
            clientInfo = tmp;
            clientInfo.setMessage("200");
            client.send(new Gson().toJson(clientInfo, clientInfo.class));
            ...
            try {
                ...
                if (Number == 405) {
                    Minecraft mc = Minecraft.getInstance();
                    if (mc.player != null)
                        mc.player.sendSystemMessage(Component.literal("发送的消息长度超过1950").setStyle(Style.EMPTY.withColor(0xFF0000)));
                } else {
                    dgStrength.setBMaxStrength(Number);
                }
            } catch (NumberFormatException ignored) {
                // 忽略无效数字
            }
